...
module Prism
  ...
  #
  class Pattern
    # This is the parent class of all errors that can be raised by the pattern
    # virtual machine.
    ...
    class UnsupportedPatternError < CompilationError
      def initialize(repr)
        super(<<~ERROR)
    ...
    class IncompatibleFieldError < CompilationError
    ...
    # of the two patterns.
    def or(other)
      Pattern.new("#{pattern} | #{other.pattern}")
    end
    alias | or
    ...
    def visit_array_pattern_node(node)
      if !node.rest.nil? || node.posts.any?
        raise UnsupportedPatternError, node.inspect
      end
    ...
    def visit_hash_pattern_node(node)
      ...
      if node.constant
        ...
        unless constant < Node
          raise UnsupportedPatternError, node.constant.inspect
        end
      ...
      node.elements.each do |element|
        ...
        unless element.is_a?(AssocNode)
          raise UnsupportedPatternError, element.inspect
        end

        # The only way to have a pattern matching key is for it to be a symbol.
        # So in this case we are safe to assume the interface.
        key = element.key.value.to_sym

        if (field = constant&.field(key))
          case field[:type]
          when :constant_list, :node_list
            if element.value.is_a?(ArrayPatternNode)
              @checking_type = false
              pushfield(key)
              visit(element.value)
              pop
            else
              raise IncompatibleFieldError.new("#{constant}##{key}", element.value.inspect)
            end
          else
            pushfield(key)
            visit(element.value)
    ...
    # Define a method missing to provide better error messages when a visit
    # method is not yet implemented.
    def method_missing(name, *args)
      if name.start_with?("visit_")
        raise UnsupportedPatternError, args.first.inspect
      else
        super
      end
    end
